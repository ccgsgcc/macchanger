...
def get_random_mac_address():
    """Сгенерировать и вернуть MAC-адрес, йоу"""
...
def get_current_mac_address(iface):
    ...
    output = subprocess.check_output("ifconfig {iface}", shell=True).decode()
    return re.search("ether (.+) ", output).group().split()[1].strip()

def change_mac_address(iface, new_mac_address):
    # disable the network interface
    subprocess.check_output("ifconfig {iface} down", shell=True)
    # change the MAC
    subprocess.check_output("ifconfig {iface} hw ether {new_mac_address}", shell=True)
    # enable the network interface again
    subprocess.check_output("ifconfig {iface} up", shell=True)
